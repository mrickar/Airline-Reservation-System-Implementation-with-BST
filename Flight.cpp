...
bool Flight::addTicket(const Ticket &ticket) {
    ...
    if(ticket.getTicketType()==economy)
    {
        ...
        for(int i=0;i<sz;i++)
        {
            if(tickets[i].getTicketType()==economy) cnt++;
        }
        if(cnt==getEconomyCapacity())
        {
            return false;
        }
    }
    else if(ticket.getTicketType()==business)
    {
        ...
        for(int i=0;i<sz;i++)
        {
            if(tickets[i].getTicketType()==business) cnt++;
        }
        if(cnt==getBusinessCapacity())
        {
            return false;
        }
    }
...
bool Flight::operator<(const Flight &rhs) const {
    /* TODO */
    return getFlightCode()<rhs.getFlightCode();
}
